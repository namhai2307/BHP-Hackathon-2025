# Mooring Data Generator Flask API

A Flask web application that generates and serves mooring port data using the `mooring-data-generator` package.
...
### Port Data
- **GET /api/port** - Generate and retrieve random port data
- **GET /api/port/berths** - Get detailed information for all berths
- **GET /api/port/berth/<berth_name>** - Get specific berth details
- **POST /api/port/update** - Update port data (simulates real-time updates)
- **GET /api/port/raw** - Get raw port data as string
- **GET /api/port/statistics** - Get statistics about the current port

## Example Usage

```bash
# Generate port data
curl http://localhost:5000/api/port

# Get all berths with details
curl http://localhost:5000/api/port/berths

# Update the port data
curl -X POST http://localhost:5000/api/port/update

# Get statistics
curl http://localhost:5000/api/port/statistics
```

---

# Original Mooring Data Generator CLI

A simple script to generate fake mooring data for use in a hackathon.
This script will send data payloads to an endpoint to simulate the data which might exist.
